            ...
            <template #item="{ element, index }">
              <transition-group name="fade" tag="div">
                <template v-if="islayoutComponent(element.type)">
                  <AntdWidgetLayoutForm :element="element" :index="index" @click.stop="handleItemClick(element)"></AntdWidgetLayoutForm>
                </template>
                <template v-else>
                  <AntdWidgetFormItem
                    v-if="element.key"
...
export default defineComponent({
  ...
  setup() {
    ...
    const handleCopyClick = (index: number, list: any[]) => {
      ...
      const oldList = JSON.parse(JSON.stringify(getWidgetForm().list))
    ...
    const handleMoveAdd = (event: any) => {
      ...
      const key = v4().replaceAll('-', '')
      const list = JSON.parse(JSON.stringify(getWidgetForm().list))
      ...
      updateWidgetForm({ ...widgetForm, list })
